...
ONNXDriver::ONNXDriver(
    ...
    std::shared_ptr<H1_2_Driver> unitree_driver
) : 
...
absl::Status ONNXDriver::set_control_mode(h1_2::constants::HighLevelControlMode mode) {
    std::lock_guard<std::mutex> lock(mutex);
    control_mode = mode;
    return absl::OkStatus();
};

const h1_2::constants::HighLevelControlMode ONNXDriver::get_control_mode() {
    std::lock_guard<std::mutex> lock(mutex);
    return control_mode;
};
...
absl::Status ONNXDriver::make_observation() {
    // Get Measurements:
    std::optional<H1_2_State> state = unitree_driver->get_state();
    if (!state)
    ...
    h1_2::constants::MotorVector<float> joint_positions;
    h1_2::constants::MotorVector<float> joint_velocities;
    ...
    // Previous Actions:
    h1_2::constants::MotorVector<float> previous_actions = Eigen::Map<h1_2::constants::MotorVector<float>>(policy_output.data());
...
H1_2_Command ONNXDriver::policy_command() {
    h1_2::constants::MotorVector<float> actions = Eigen::Map<h1_2::constants::MotorVector<float>>(policy_output.data());
    h1_2::constants::MotorVector<float> position_setpoints = default_position + master_gain * action_scale * actions;

    H1_2_Command command = h1_2::utilities::default_position_command();
...
void ONNXDriver::policy_callback() {
    ...
    auto state = unitree_driver->get_state();
    switch (control_mode) {
        case h1_2::constants::HighLevelControlMode::DEFAULT:
            command = h1_2::utilities::default_position_command();
            for (size_t i = 0; i < h1_2::constants::num_joints; ++i) {
                auto& motor_command = command.motor_cmd[i];
            ...
            break;
        case h1_2::constants::HighLevelControlMode::HOLD_POSITION:
            if (state) {
                command = h1_2::utilities::hold_position_command(*state, kp, kd);
            }
            else {
                command = h1_2::utilities::damping_command();
                control_mode = h1_2::constants::HighLevelControlMode::DAMPING;
            }
            break;
        case h1_2::constants::HighLevelControlMode::DAMPING:
            command = h1_2::utilities::damping_command();
            break;
        case h1_2::constants::HighLevelControlMode::POLICY:
            command = this->policy_command();
            break;
        case h1_2::constants::HighLevelControlMode::DISABLE:
            command = h1_2::utilities::disable_command();
            break;
    }
