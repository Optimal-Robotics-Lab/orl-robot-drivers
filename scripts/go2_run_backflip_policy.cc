...
#include "src/go2/lib/policies/backflip_control_policy.h"
...
int main(int argc, char * argv[]) {
    ...
    auto PolicyDriver = std::make_shared<BackflipControlPolicy<decltype(FilterDriver)>>(
        options, 
        onnx_model_path, 
        RobotDriver, 
        std::move(FilterDriver)
    );
    ...
    while (!stop_token.stop_requested() && is_running) {
        ...
        if (!state) {
            ...
            if (PolicyDriver->get_control_mode() == go2::constants::HighLevelControlMode::POLICY) {
                auto phase = PolicyDriver->get_phase();
